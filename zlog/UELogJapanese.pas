...
procedure TformELogJarl.FormCreate(Sender: TObject);
begin
   InitializeFields;
end;

procedure TformELogJarl.RemoveBlankLines(M: TMemo);
var
   i: integer;
begin
   i := M.Lines.Count - 1;
...
procedure TformELogJarl.InitializeFields;
var
   ini: TIniFile;
begin
...
procedure TformELogJarl.buttonCreateLogClick(Sender: TObject);
var
   f: TextFile;
   fname: string;
begin
...
procedure TformELogJarl.buttonSaveClick(Sender: TObject);
var
   ini: TIniFile;
begin
   ini := TIniFile.Create(ChangeFileExt(Application.ExeName, '.ini'));
...
procedure TformELogJarl.buttonCancelClick(Sender: TObject);
begin
   Close;
end;
...
procedure TformELogJarl.WriteSummarySheet(var f: TextFile);
var
   nFdCoeff: Integer;
begin
...
procedure TformELogJarl.WriteLogSheet(var f: TextFile);
var
   i: Integer;
   s: string;
begin
...
function TformELogJarl.FormatQSO(q: TQSO): string;
var
   slLine: TStringList;
begin
