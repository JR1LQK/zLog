...
type
  ...
  TRRRecord = packed record
    ...
    case integer of // depending on rrtype, one of these structures is filled
      ...
      DnsQueryA      : (adress    : tadress;
                        ipnummer  : tname);    // interpreted
      DnsQueryAAAA   : (aaaa      : TAAAA;
                        ip6nummer : tname);    // interpreted
  ...
  protected
    ...
    FResponseBuf                : array [0..511*8] of char;  // 4k genug? irgendwie wäre mir dynamisch ja lieber...
...
begin
  frrcache.lastid:=-1;
  FResponseLen:=FWSocket.Receive(@FResponseBuf,SizeOf(FResponseBuf));
...
begin
  ...
  case fdatacache.items[index].rrtype of
    ...
    DnsQueryA      : begin
                       ...
                       result.ipnummer:=WSocket_inet_ntoa(tinaddr(result.adress));
                     end;
    DnsQueryMX     : begin
    ...
    DnsQueryAAAA   : begin
                       ...
                       result.ip6nummer:=format('%p:%p',[ pointer(result.aaaa[0]),pointer(result.aaaa[2]) ]);
                     end;
    DnsQuerySOA    : begin
